...
const DEPLOY_MODEL_NODE_COUNT_REGION_TAG = 'vision_classification_deploy_model_node_count';
const LOCATION = 'us-central1';

describe('Automl Vision Classification Model Tests', () => {
  const client = new AutoMlClient();

  it('should undeploy and deploy the model', async () => {
  ...
  it('should undeploy and deploy the model with a node count', async () => {
    const projectId = await client.getProjectId();
    ...
    const deploy_output = execSync(
      `node ${DEPLOY_MODEL_NODE_COUNT_REGION_TAG}.js ${projectId} ${LOCATION} ${MODEL_ID}`
    );
    assert.match(deploy_output, /Model deployment finished/);
  });
});
